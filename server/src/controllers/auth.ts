...
export const register = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if ((error as CustomError).status === 409) {
      next(new CustomError((error as CustomError).message, 409, 'DUPLICATE_USER_KEY'));
      return;
    }
...
export const login = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if ((error as CustomError).status === 401) {
      next(new CustomError((error as CustomError).message, 401, 'INVALID_CREDENTIALS'));
      return;
    }
...
export const newAccessTokenFromRefreshToken = async (
  ...
  try {
    ...
    next(new CustomError((error as Error).message, 500, 'NEW_ACCESS_TOKEN_ERROR'));
  }
};
...
export const forgotPassword = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    next(new CustomError((error as Error).message, 500, 'FORGOT_PASSWORD_ERROR'));
  }
};
...
export const resetPassword = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    next(new CustomError((error as Error).message, 500, 'RESET_PASSWORD_ERROR'));
  }
};
